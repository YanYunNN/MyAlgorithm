...
public class SpinLockTest {
    ...
    public class TicketLock {
        // 队列票据(当前排队号码)
        private final AtomicInteger queueNum = new AtomicInteger();
        // 出队票据(当前需等待号码)
        private final AtomicInteger dueueNum = new AtomicInteger();

        //因为获得自己的号码之后，是可以对号码进行更改的，这就造成系统紊乱，锁不能及时释放。
        // 这时候就需要有一个能确保每个人按会着自己号码排队办业务的角色
        private final ThreadLocal<Integer> ticketLocal = new ThreadLocal<>();
